...
import { gamesAPI, Game, getAuthToken, libraryAPI, reviewsAPI, Review } from '@/lib/api';
...
export default function GameDetailPage() {
  ...
  useEffect(() => {
    ...
    loadGameDetails();
    loadPlatforms();
    loadReviewsAndSummary(); // Combined load
    if (loggedIn) {
      checkLibraryStatus();
    }
  }, [gameId]);
  ...
  // --- NEW: Load Reviews for Stats & AI Summary ---
  const loadReviewsAndSummary = async () => {
    try {
      setLoadingSummary(true);
      // Fetch first 50 reviews for analysis
      const reviewsData = await reviewsAPI.getGameReviews(gameId, 0, 50);
      const reviewList: Review[] = Array.isArray(reviewsData) ? reviewsData : (reviewsData.content || []);
      ...
      if (reviewList.length > 0) {
        ...
        // 2. Call Python Chat Server for Summary
        // Check if localhost:5001 is available (AI Service)
        try {
          const reviewTexts = reviewList.map(r => r.reviewText);
          ...
          if (response.ok) {
            ...
            setAiSummary("AI Service is currently offline. (Make sure chat_server.py is running!)");
          }

          // 3. Fetch Sentiment Analysis
          const sentimentResponse = await fetch('http://localhost:5001/sentiment', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          console.error("AI Server Error:", serverErr);
          setAiSummary("AI Analysis unavailable (Python server not responding).");
        }

      } else {
        setAverageRating("N/A");
        setAiSummary("Not enough reviews yet for AI analysis.");
      }
    } catch (err) {
      console.log("Error loading reviews for summary:", err);
    } finally {
      ...
      <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 pt-28">
            ...
            <div className="flex flex-col md:flex-row gap-8 relative z-10">
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-xl border border-white/5 hover:border-purple-500/30 transition-colors col-span-2 md:col-span-1">
                    <p className="text-purple-400 text-xs uppercase tracking-wider mb-1">User Rating</p>
                    <div className="flex items-center gap-2">
                      <span className={`text-xl font-bold ${averageRating !== "N/A" && parseFloat(averageRating) >= 4.0 ? "text-green-400" : "text-white"}`}>
                        {averageRating}
                      </span>
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-xl border border-white/5 hover:border-purple-500/30 transition-colors col-span-2 md:col-span-1">
                    <p className="text-purple-400 text-xs uppercase tracking-wider mb-1">Sentiment (AI)</p>
                    <div className="flex items-center gap-2">
                      <span className={`text-xl font-bold ${sentiment?.score >= 6 ? 'text-green-400' : sentiment?.score >= 4 ? 'text-yellow-400' : 'text-gray-400'}`}>
                ...
                {aiSummary && (
                  <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-purple-900/10 to-slate-900 border border-purple-500/30 relative overflow-hidden group">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 w-full">
                        <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-2">
                          AI Review Summary
                          <span className="px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 text-[10px] uppercase tracking-wide">
                            GPT-5.1
                          </span>
                        </h3>
                        {loadingSummary ? (
                          <div className="text-gray-400 animate-pulse text-sm">Analysing {game.title} reviews... connecting to neural core...</div>
                        ) : (
          ...
          <div className="glass rounded-2xl p-8 border border-purple-500/20 relative overflow-hidden">
            ...
            <ReviewSection gameId={gameId} totalReviews={sentiment?.total} />
          </div>
        </div>
      </div>
    </>
